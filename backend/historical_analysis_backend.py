...
def get_summary_llm():
    """Get configured LLM for summary generation"""
    if USE_OLLAMA:
        return LLM(model=f"ollama/{OLLAMA_SUMMARY}", base_url="http://localhost:11434")
    else:
        if not GOOGLE_API_KEY:
            return None
        return LLM(
            model="gemini/gemini-2.5-flash", temperature=0.3, api_key=GOOGLE_API_KEY
        )
...
def generate_data_summary_with_llm(section_name: str, data: Dict[str, Any]) -> str:
    """
    Generate a 2-3 line summary using Gemini or Ollama
    ...
    try:
        ...
        # Use CrewAI LLM for consistent interface
...
def extract_summary_data(metrics: Dict, data_df) -> Dict[str, Dict]:
    ...
    summary_data["Response Time Analysis"] = {
        ...
        "mttr_90th": metrics.get("mttr_analysis", {})
        .get("percentiles", {})
        .get("90th", "N/A"),
    ...
    if "vip_analysis" in metrics:
        ...
        summary_data["VIP User Distribution"] = {
            ...
            "non_vip_avg_mttr": (
                f"{non_vip_mttr:.1f}" if non_vip_mttr is not None else "N/A"
            ),
        }
