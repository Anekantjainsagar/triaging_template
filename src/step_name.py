...
if __name__ == "__main__":
    generator = IntelligentStepNameGenerator()

    # Test cases from Rule#014
    test_cases = [
        (
            "Take Remediation Actions",
            "If unauthorized access is confirmed, lock account",
            1,
            "Rule#014",
        ),
        ("Investigation Step 2", "Complete 2.0 and document findings", 2, "Rule#014"),
        ("Investigation Step 3", "Gather details of all users", 3, "Rule#014"),
        (
            "Investigation Step 4",
            "Username, Role Assigned, Time of Assignment",
            4,
            "Rule#014",
        ),
        (
            "Investigation Step 5",
            "Determine if the assigned role is high-risk",
            5,
            "Rule#014",
        ),
        (
            "Investigation Step 6",
            "Look for unusual sign-in patterns by the assigned user",
            6,
            "Rule#014",
        ),
        (
            "Investigation Step 7",
            "Validate if the assigning user had legitimate access",
            7,
            "Rule#014",
        ),
        ("Investigation Step 8", "if suspicious, escalate to L3/IT", 8, "Rule#014"),
        (
            "Investigation Step 9",
            "Reach out to Network/EDR teams to block IP",
            9,
            "Rule#014",
        ),
        (
            "Investigation Step 10",
            "After all investigation document the steps taken",
            10,
            "Rule#014",
        ),
    ]
    ...
    print("=" * 80)
    print("STEP NAME GENERATION TEST")
    print("=" * 80)

    for raw_name, explanation, step_num, context in test_cases:
        generated = generator.generate_step_name(
            raw_name, explanation, step_num, context
        )
        print(f"\n{step_num}. Original: {raw_name}")
        print(f"   Generated: {generated}")
        print(f"   Context: {explanation[:60]}...")
