...
def generate_blank_triaging_template_csv(
    ...
    """
    Generate CSV template that follows the LEARNED structure from original template.
    Each step should show CONTEXT and CONDITIONAL LOGIC.
    """
    import csv
    ...
    # Header row
    writer.writerow(["Sr.No.", "Inputs Required", "INPUT details", "Instructions"])
    ...
    # Add each AI-generated step with CONTEXT and CONDITIONAL LOGIC
    for step in triaging_steps:
        ...
        if decision_point:
            instructions += f" | DECISION POINT: {decision_point}"

        # Add expected output
        if expected_output:
            instructions += f" | EXPECTED: {expected_output}"

        # Add KQL query
        if kql_query and kql_query.strip():
            # Format KQL on single line for CSV
            kql_formatted = kql_query.replace("\n", " ").replace("  ", " ")
            instructions += f" | RUN QUERY: {kql_formatted}"

        # Clean up for CSV (replace multiple spaces, trim)
        instructions = " ".join(instructions.split())

        # Write the row
        writer.writerow(
            [sr_no, step_name, "", instructions]  # User fills this during investigation
        )
    ...
    writer.writerow(
        [
            ...
            "",
            f"True Positive / False Positive / Benign Positive (Historical: {rule_history.get('fp_rate', 50)}% FP, {rule_history.get('tp_rate', 50)}% TP)",
        ]
    )
    sr_no += 1
    ...
    writer.writerow(
        [
            ...
            "Confidence Level",
            "",
            "High / Medium / Low - based on strength of evidence from all previous steps",
        ]
    )
    sr_no += 1
    ...
    writer.writerow(
        [
            ...
            "Justification",
            "",
            "Provide detailed reasoning referencing specific findings from steps above",
        ]
    )
    sr_no += 1
    ...
    writer.writerow(
        [
            ...
            "Actions Taken",
            "",
            "List all investigative actions: queries run, logs checked, users contacted, escalations made",
        ]
    )
    sr_no += 1
    ...
    writer.writerow(
        [
            ...
            "Escalation Required?",
            "",
            "Yes/No - If yes, specify: L3 SOC / IT Team / Security Manager / Incident Response",
        ]
    )

    # Add reference section
    writer.writerow([])
    writer.writerow(["", "HISTORICAL REFERENCE DATA", "", ""])
    ...
    writer.writerow(
        [
            ...
            f"{rule_number} has {rule_history.get('total_incidents', 0)} historical incidents for pattern analysis",
        ]
    )
    ...
    writer.writerow(
        [
            "",
            "False Positive Rate",
            f"{rule_history.get('fp_rate', 0)}%",
            f"{rule_history.get('false_positives', 0)} of {rule_history.get('total_incidents', 0)} incidents were False Positives",
        ]
    )
    ...
    writer.writerow(
        [
            "",
            "True Positive Rate",
            f"{rule_history.get('tp_rate', 0)}%",
            f"{rule_history.get('true_positives', 0)} of {rule_history.get('total_incidents', 0)} incidents were True Positives",
        ]
    )
...
def get_triaging_template(rule_number: str) -> str:
    ...
    try:
        if file_ext == ".csv":
            ...
            return parse_dataframe_template_to_structured_text(df)
        else:
...
def parse_dataframe_template_to_structured_text(df: pd.DataFrame) -> str:
    """Parse DataFrame template (Excel) into structured text."""
    return parse_csv_template_to_structured_text(df)  # Use same logic

def generate_blank_triaging_template(rule_number: str, triaging_steps: list) -> str:
    """
    Generate a blank triaging template with structure from AI-generated steps.
    This template can be exported for manual use.
    """
    timestamp = datetime.now().strftime("%Y-%m-%d %H:%M:%S")

    template = f"""
================================================================================
SECURITY INCIDENT TRIAGING TEMPLATE
Rule: {rule_number}
Generated: {timestamp}
================================================================================

INCIDENT DETAILS (TO BE FILLED)
--------------------------------------------------------------------------------
Incident Number:        _______________
Reported Time:          _______________
Priority:               _______________
Data Connector:         _______________
Affected User(s):       _______________
VIP Users Involved:     [ ] Yes  [ ] No
Shift Engineer:         _______________

"""

    # Add each triaging step with structure
    for i, step in enumerate(triaging_steps, 1):
        step_name = step.get("step_name", f"Step {i}")
        explanation = step.get("explanation", "No description available")
        kql_query = step.get("kql_query", "")
...
"""
        ...
        template += f"""Findings:
[Document your investigation findings here]

Actions Taken:
[Document any actions performed]

Result: [ ] Complete  [ ] Pending  [ ] Blocked
...
Justification:
[Provide detailed justification for your classification]


Escalation Required: [ ] Yes  [ ] No

If Yes, Escalate to: _______________

Actions Recommended:
[List any recommended actions or follow-ups]


TIMELINE
--------------------------------------------------------------------------------
Investigation Start:     _______________
Investigation End:       _______________
Total Time (MTTR):       _______________ minutes


APPROVALS
--------------------------------------------------------------------------------
Analyzed By:            _______________
